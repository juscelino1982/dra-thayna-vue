 ...
 *
 * CRUD completo de agendamentos com sincronização automática
 * para Google Calendar quando habilitado
 */

import express, { Request, Response } from 'express'
import { prisma } from '../config/prisma'
import { syncAppointmentToGoogle, unsyncAppointmentFromGoogle, getUserIntegration } from '../services/google-calendar'
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Tenta sincronizar com Google Calendar se o usuário tem integração ativa
    try {
      const integration = await getUserIntegration(finalUserId)
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      const integration = await getUserIntegration(appointment.userId)
      if (integration && integration.isActive && appointment.googleEventId) {
        await syncAppointmentToGoogle(appointment.id)
      }
    } catch (syncError) {
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Remove do Google Calendar se estiver sincronizado
    if (appointment.googleEventId) {
      try {
        await unsyncAppointmentFromGoogle(id)
      } catch (syncError) {
        console.error('Erro ao remover do Google Calendar:', syncError)
      }
    }
...
router.post('/:id/cancel', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Erro ao atualizar cancelamento:', syncError)
    }
